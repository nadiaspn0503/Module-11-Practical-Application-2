...
It should be noted that the vehicles dataset and the file with the coding for this project are both too large to automatically display in GitHub. Both files must be downloaded in order to view them. The file with the coding specifically must be downloaded in Visual Studio Code and the workspace must be trusted in order to completely view the coding and plots.
